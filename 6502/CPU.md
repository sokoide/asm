...
> **コラム: 6502のBCDモードはなぜ生き残ったのか──デシマル演算の栄光と終焉**
>
> 6502のDフラグ（Decimal Mode）をSEDで有効にすると、ADC / SBCが自動的にBCD（2進化10進数）演算を行う。
> これは電卓やPOSレジスタなど、0.1 + 0.2 = 0.3を正確に表現する必要がある業務用途のためだ。
> 2進小数ではこの計算に誤差が生じるが、BCDなら桁ごとに10進処理するため誤差がない。
> IEEE 754浮動小数点標準の普及とCPUの高速化によりBCD専用回路の価値は薄れ、
> ARM64やRISC-VにはBCD命令はなく、x86でもDAA/DASのみが残る。
> 6502のBCDモードは、CPUがまだ「汎用」だった時代の名残である。
...
> **コラム: なぜ6502はこれほど安かったのか──トランジスタ数削減の執念**
>
> MOS 6502の開発当時、競合のMotorola 6800は約$300だったのに対し、6502はわずか$25で発売された。
> その秘密はデコードPLAの高密度化とトランジスタ数の徹底的な削減（3510個）にある。
> ページ境界クロス時の追加サイクル（+1サイクル）はパイプライン先行実行を切り捨てることで実現したコスト削減の産物であり、
> CPUの動作サイクルが不規則な理由は、この実装の単純さを優先する設計哲学に由来する。
