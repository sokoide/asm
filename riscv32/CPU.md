...
> **コラム: なぜRISC-VにZeroレジスタ（x0）があるのか──即値のジレンマを解く設計哲学**
>
> RISC-Vのx0は読み出すと常に0を返し、書き込んでも無視される特殊レジスタである。
> `li rd, 0` のような頻出操作を `addi rd, x0, 0` と1命令で表現するために存在する。
> ARM64のXZR、PowerPCのR0（RAフィールドで0扱い）など似た発想は他にもあるが、
> RISC-Vはこれを最も一貫した形でISAの中心に据えている。
...
> レジスタ指定シフトでは `rs2` の下位5ビットのみを使用。即値シフトの `imm` 範囲は 0〜31。
