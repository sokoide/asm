...
| Z      | Zero     | 結果が 0                             |
| C      | Carry    | 加算では繰り上がり、減算では C=1 がボローなしを示す |
...
> **コラム: なぜARMにPUSH/POPがないのか──RISC哲学の本質**
> 
> x86の `PUSH AX` やZ80の `PUSH BC` は1命令で「スタックポインタを減らして書き込む」という2操作を同時に行う。
> ARM64には専用のPUSH/POP命令がなく、`STP X29, X30, [SP, #-16]!` のようにアドレス計算・書き込み・ポインタ更新をすべて明示的に書く。
> これは「1命令は1操作」というRISC哲学による設計判断であり、ハードウェアの単純化とパイプライン効率を優先している。
> このリポジトリ内のx86_16（CISC）と比較すると、アーキテクチャ思想の差が実感できる。
