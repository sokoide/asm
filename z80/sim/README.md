...
`z80asm` + Go 製 Z80 シミュレータ(`simz80`)によるハンズオン教材。CP/M BDOS 呼び出し規約を使用。
...
Z80 は「メイン」と「裏(shadow/alternate)」の 2 組のレジスタを持つ:
...
- **終了**: プログラムは `RET` で終了。SP に積まれた `0x0000` に戻るとシミュレータが停止する
