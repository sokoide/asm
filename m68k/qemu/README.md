...
**ポイント**: 除算の結果は、D0 の下位16ビットに商、上位16ビットに余りが格納される。
...
**ポイント**: POP時はレジスタリストを逆順に指定することで、正しい順序に復元できる。
...
| `blt label`        | より小さい（Below）ならジャンプ（符号なし）        |
| `bgt label`        | より大きい（Above）ならジャンプ（符号なし）        |
...
3. REG_DATA_LEN に 1 を設定（1バイト読み込み）
...
**応用例**:
- AND: 特定ビットのマスク（`and.l #0xFF, %d0` → 下位8ビットだけ抽出）
- OR: 特定ビットのセット（`or.l #0x80, %d0` → 最上位ビットを1に）
- XOR: ビット反転（`eor.l #0xFF, %d0` → 下位8ビットを反転）

**表示ルーチン**: 16進数と2進数の表示サブルーチンを実装。シフト演算で各ビットを1つずつ取り出す。
...
| 命令               | 動作                              |
| :---               | :---                              |
| `move.l src, dst`  | src → dst（32-bit）               |
| `move.w src, dst`  | src → dst（16-bit）               |
| `move.b src, dst`  | src → dst（8-bit）                |
| `move.l #val, reg` | 即値をレジスタにロード            |
| `push.l src`       | スタックに保存（SP -= 4）         |
| `pop.l dst`        | スタックから復元（SP += 4）       |
| `xchg.l a, b`      | a と b を交換                     |
| `lea addr, reg`    | 実効アドレスを計算して reg に格納 |
...
| `dec.l dst`        | dst--                         |
| `neg.l dst`        | dst = -dst                    |
| `addx.l src, dst`  | dst += src + X                |
| `subx.l src, dst`  | dst -= src - X                |
| `mulu.w src, dst`  | 符号なし乗算（D0 = D0 × src） |
| `divu.w src, dst`  | 符号なし除算（D0 = D0 ÷ src） |
| `muls.w src, dst`  | 符号付き乗算                  |
| `divs.w src, dst`  | 符号付き除算                  |

### 論理演算

| 命令               | 動作              |
| :---               | :---              |
| `and.l src, dst`   | dst = dst AND src |
| `andi.l #val, dst` | dst = dst AND val |
| `or.l src, dst`    | dst = dst OR src  |
| `ori.l #val, dst`  | dst = dst OR val  |
| `eor.l src, dst`   | dst = dst XOR src |
| `eori.l #val, dst` | dst = dst XOR val |
| `not.l dst`        | dst = NOT dst     |

### シフト・ローテート

| 命令                 | 動作                                    |
| :---                 | :---                                    |
| `lsl.l #count, dst`  | 左シフト（Logical Shift Left）          |
| `lsr.l #count, dst`  | 右シフト（Logical Shift Right）         |
| `asr.l #count, dst`  | 算術右シフト（ Arithmetic Shift Right） |
| `rol.l #count, dst`  | 左ローテート                            |
| `ror.l #count, dst`  | 右ローテート                            |
| `roxl.l #count, dst` | 左ローテート（Xを含む）                 |
| `roxr.l #count, dst` | 右ローテート（Xを含む）                 |

### 比較・分岐

| 命令               | 動作                      | 条件         |
| :---               | :---                      | :---         |
| `cmp.l src, dst`   | dst - src（フラグ更新）   | -            |
| `cmpi.l #val, dst` | dst - val（フラグ更新）   | -            |
| `tst.l dst`        | dst - 0（フラグ更新）     | -            |
| `bra label`        | 無条件ジャンプ            | -            |
| `beq label`        | 等しい                    | ZF=1         |
| `bne label`        | 等しくない                | ZF=0         |
| `bgt label`        | より大きい（符号付き）    | ZF=0 ∧ SF=OF |
...
| `bcc label`        | 以上（符号なし）          | CF=0         |
| `bls label`        | 以下（符号なし）          | CF=1 ∨ ZF=1  |
| `bcs label`        | より小さい（符号なし）    | CF=1         |
| `dbra reg, label`  | reg--, reg≠0 ならジャンプ | -            |

### 文字列操作（実装）

| 命令                 | 動作              |
| :---                 | :---              |
| `move.b (%a0)+, %d0` | [A0] → D0, A0++   |
| `move.b %d0, (%a1)+` | D0 → [A1], A1++   |
| `tst.b (%a0)+`       | [A0] をテスト     |
| `cmp.b (%a1), %d0`   | D0 と [A1] を比較 |
...
| :---                       | :---                                               |
| `bsr label`                | サブルーチン呼び出し（リターンアドレスを PUSH）    |
| `jsr label`                | ジャンプしてサブルーチン呼び出し（絶対アドレス）   |
| `rts`                      | サブルーチンから復帰（スタックからアドレスを POP） |
| `rtd #offset`              | 相対オフセット付き復帰                             |
| `movem.l reg_list, -(%sp)` | レジスタをスタックに保存                           |
| `movem.l (%sp)+, reg_list` | スタックからレジスタを復元                         |

### スタック

| 命令                  | 動作                                 |
| :---                  | :---                                 |
| `move.l src, -(%sp)`  | スタックにプッシュ（先に SP を減算） |
| `move.l (%sp)+, dst`  | スタックからポップ（後に SP を増加） |
| `pea.l addr`          | 実効アドレスをスタックにプッシュ     |
| `link.w #offset, %fp` | スタックフレーム作成                 |
| `unlk %fp`            | スタックフレーム解除                 |
...
| `reset`          | リセット（システム再起動）             |
| `stop #value`    | ストップモードに入る                   |
| `illegal`        | 不正命令を生成（例外）                 |
| `chk.w src, dst` | レジスタ値のチェック（範囲外なら例外） |
| `trap #vector`   | トラップ例外を発生                     |

### 状態レジスタ

| 命令                | 動作                               |
| :---                | :---                               |
| `ori.w #mask, %sr`  | ステータスレジスタのビットをセット |
| `andi.w #mask, %sr` | ステータスレジスタのビットをクリア |
| `move.w %sr, %d0`   | ステータスレジスタを読み込む       |
| `move.w %d0, %sr`   | ステータスレジスタに書き込む       |
