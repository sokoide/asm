...
> **コラム: なぜPowerPCの減算はオペランド順が逆なのか──「subtract from」の発想**
>
> `subf rd, ra, rb` は `rd = rb - ra` と、直感と逆のオペランド順を持つ。
> subf は "subtract from" の略で「raをrbから引く」という意味。この命名はIBMがSystem/360時代から継承してきた伝統による。
> さらにPowerPCには `subfic`、`subfze` など多様な減算亜種が存在する。これらはコンパイラが定数削減などの最適化を効率的に行えるようにするための設計で、
> PowerPCが人間の可読性よりコンパイラ最適化を重視していることを示している。
> ARM64の `sub` やRISC-Vの `sub` と比較すると、同じRISCでも設計思想の違いが浮き彫りになる。
