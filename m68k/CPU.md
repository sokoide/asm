...
| `cmpa.l src, ay`      | ay - src（フラグのみ、アドレス比較） |
...
> **コラム: なぜM68000のアドレッシングモードはこんなに豊富なのか──CISCの到達点**
>
> M68000は14種類ものアドレッシングモードを持ち、`move.l 16(%a0, %d1.l), %d0` のように
> ベース＋インデックス×スケール＋変位のアドレス計算を1命令で書ける。
> これはRISCが「1命令1操作」を採用したのとは正反対のCISC哲学の到達点を示している。
> デコード回路は複雑になるがコード密度は高く、メモリが高価だった1980年代初頭には理にかなった設計だった。
> この複雑さがパイプライン実装の障壁となり、後のRISCの隆盛を招く一因となった。
